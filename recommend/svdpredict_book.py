...
data = Dataset.load_from_file('./recommend/rating_book.csv', reader=reader)
...
with open ('./recommend/user_book.csv', newline='', encoding='utf-8-sig') as csvfile:
    reader = csv.DictReader(csvfile)
    user = []
    for row in reader:
        user.append(row['userId'])

with open ('./recommend/book.csv', newline='', encoding='utf-8-sig') as csvfile:
    reader = csv.DictReader(csvfile)
    book = []
    for row in reader:
        book.append(row['bookId'])


# 保存predictions到文件中（保存真实值和预测值）
with open('./recommend/svd_predict_book.csv', 'w', newline='', encoding='utf-8-sig') as f:
    ...
    csv.DictWriter(f, fieldnames=['user', 'book', 'Rate', 'original_rate']).writeheader()
    for uid, iid, r_ui, est, _ in predictions:
        user_id = user[int(uid)-1]
        book_id = book[int(iid)-1]
        csv.DictWriter(f, fieldnames=['user', 'book', 'Rate', 'original_rate']).writerow({'user': user_id, 'book': book_id, 'Rate': est, 'original_rate': r_ui})
